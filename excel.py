...
for row in range(1, 16):
    ...
    for col in range(0,3):
        buf = str(worksheet.cell(row, col).value)
        if ';' in buf:
            process[col] = buf
        else:
...
for row in range(0, len(processes)):
    ...
    if processes[row][2] == "0": # Если процесс не зависит от других
        ...
        print("{} зависит от процессов: {}".
              format(processes[row][0], list_dependent))
        
        # Список временных задержек зависимых процессов
        times_dependent = [ int(processes[int(ID)-1][3])
                            for ID in list_dependent ]
        
        # Расчет полного времени текущего процесса
        processes[row][3] = int(processes[row][1]) + max(times_dependent)
        
        print("Список временных задержек для {}: {}"
              .format(processes[row][0], times_dependent))
